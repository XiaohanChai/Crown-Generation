...
class UNet3DModel(nn.Module):
    ...
    def forward_as_middle(self, h, doctree, timesteps, label, context):
        # --- 添加以下补丁代码 ---
        # 修复 h 与 octree 节点数量不匹配的问题
        # 获取 octree 在 full_depth 层的实际节点索引数量
        idx_data = doctree.octree.xyzb(self.full_depth) 
        target_size = idx_data[0].shape[0] # idx_data 是 (x, y, z, b)，取第一个维度的长度
        
        if h.shape[0] != target_size:
            print(f"WARNING: shape mismatch in forward_as_middle. h: {h.shape[0]}, octree nodes: {target_size}. Adjusting.")
            if h.shape[0] < target_size:
                # 如果 h 小于 octree 节点数，填充 0
                padding = torch.zeros((target_size - h.shape[0], h.shape[1]), device=h.device, dtype=h.dtype)
                h = torch.cat([h, padding], dim=0)
            else:
                # 如果 h 大于 octree 节点数，裁剪
                h = h[:target_size]
        # --- 补丁结束 ---
