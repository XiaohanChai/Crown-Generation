...
def train_main_worker(opt, model, train_loader, test_loader, visualizer):
    ...
    for iter_i in range(start_iter, total_iters):
        ...
        data = next(train_dg)
        data_first_item = {k: [v[0]] if isinstance(v, list) else torch.tensor(v[0]).to(v.device) for k, v in data.items()}
        data['iter_num'] = iter_i
        data['epoch'] = epoch
        # full_voxel, mask_voxel, removed_voxel = model.set_input(data)
        model.set_input(data)
        
        # ***FOR MYTEST***
        # model.sample(split_small=full_voxel, category = opt.category, prefix = 'results_full', ema = True, ddim_steps = 200, save_index = iter_i)
        # model.sample(split_small=mask_voxel, category = opt.category, prefix = 'results_mask', ema = True, ddim_steps = 200, save_index = iter_i)
        # model.sample(split_small=removed_voxel, category = opt.category, prefix = 'results_removed', ema = True, ddim_steps = 200, save_index = iter_i)
        ...
        if iter_i % opt.display_freq == 0:
            ...
            if opt.model == "vae":
                ...
                # for the train test batchsize mismatch
                data_first_item['iter_num'] = iter_i
                data_first_item['epoch'] = epoch
                model.set_input(data_first_item)
                try:
                    model.sample(category = category, prefix = 'results', ema = True, ddim_steps = 200, save_index = iter_i)
                except Exception as err:
                    cprint(f'[!] model.sample failed at iter {iter_i}: {err}', 'red')
...
def generate(opt, model, train_loader, test_loader):
    # this is generate func before
    # get n_epochs here
    ...
    for iter_i in range(len(test_loader)):
        if USE_TEST:
            data = next(test_dg)
            data = {k: [v[0]] if isinstance(v, list) else torch.tensor(v[0]).to(v.device) for k, v in data.items()}
        else:
            data = next(train_dg)
            data = {k: [v[0]] if isinstance(v, list) else torch.tensor(v[0]).to(v.device) for k, v in data.items()}
        print(f"filename for iter {iter_i}", data['filename'])
        data['iter_num'] = iter_i
        data['epoch'] = 0
        # full_voxel, mask_voxel, removed_voxel = model.set_input(data)
        model.set_input(data)
        ...
        if opt.model == "vae":
            ...
            try:
                model.sample(category=category, prefix='generate_results', ema=True, ddim_steps=200, save_index=iter_i)
            except Exception as err:
                cprint(f'[!] model.sample failed at iter {iter_i}: {err}', 'red')
...
if __name__ == "__main__":
    ...
    if opt.mode == 'train':
        ...
        if opt.model == "vae":
            ...
            generate(opt, model, train_loader, test_loader)
    else:
        raise ValueError
